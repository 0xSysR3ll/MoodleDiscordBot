...
async def daily_task():
    ...
    if current_date.weekday() == 0 and current_date.hour == 8:
        await send_week_events(channel)
    elif current_date.hour == 8:
        await send_today_events(channel)
...
async def ping_command(ctx): # a slash command will be created with the name "ping"
    await ctx.respond(f"Pong! ({round(bot.latency * 1000)}ms)")


async def send_today_events(channel):
    ...
    if len(events) == 0:
        await channel.send(f"Bonjour ! Il n'y a pas de cours de prévu aujourd'hui ! 📅")
    else:
        # Group events by day
        ...
        embed_title = "📅 Bonjour ! voici l'agenda d'aujourd'hui"
        if event_mode:
...
async def today_command(ctx):
    await send_today_events(ctx.channel)
...
async def tomorrow_command(ctx):
    ...
    if "PRESENTIEL" in event_name_words:
        event_mode.append("PRESENTIEL")
    if len(events) == 0:
        await ctx.respond(f"Bonjour ! Il n'y a pas de cours de prévu demain ! 📅")
    else:
        # Group events by day
        events_by_day = defaultdict(list)
        for event in events:
            events_by_day[event.begin.format('YYYY-MM-DD')].append(event)
        # Create the embed title
        embed_title = "📅 Bonjour ! Voici l'agenda pour demain"
        if event_mode:
            embed_title += f" ({', '.join(event_mode)})"
...
async def week_command(ctx):
    await send_week_events(ctx.channel)

# Refactor the logic into a separate function
async def send_week_events(channel):
    today = arrow.now().format('YYYY-MM-DD')
    ...
    if "PRESENTIEL" in event_name_words:
        event_mode.append("PRESENTIEL")
    if len(events) == 0:
        await channel.send(f"Pas de cours cette semaine ! 📅")
    else:
        # Group events by day
        events_by_day = defaultdict(list)
        for event in events:
...
async def threedays_command(ctx):
    ...
    if len(events) == 0:
        ...
        for event in events:
            events_by_day[event.begin.format('YYYY-MM-DD')].append(event)
        # Create the embed title
        embed_title = "📅 Bonjour, voici l'agenda pour les 3 prochains jours"
        if event_mode:
            embed_title += f" ({', '.join(event_mode)})"
